CREATE TABLE IF NOT EXISTS tags (
    ...
    name_norm text GENERATED ALWAYS AS (lower(regexp_replace(name, '[^a-zA-Zа-яА-ЯёЁ0-9]', '', 'g'))) STORED
);

CREATE INDEX IF NOT EXISTS idx_tags_name_norm ON tags (name_norm);

ALTER TABLE novels
    ADD COLUMN IF NOT EXISTS alt_titles jsonb DEFAULT '[]'::jsonb CHECK (jsonb_typeof(alt_titles) = 'array'),
...
CREATE INDEX IF NOT EXISTS idx_novels_alt_titles_norm_trgm ON novels USING gin (alt_titles_norm gin_trgm_ops);
