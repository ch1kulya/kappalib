...
FROM (
    ...
    FROM (
        SELECT
            ...
            chapter_num,
            created_at,
            CASE WHEN created_at - LAG(created_at) OVER (PARTITION BY novel_id ORDER BY created_at) > INTERVAL '1 hour' THEN
                1
            ELSE
                0
            END AS new_group
        FROM
            chapters) with_flags) with_groups
GROUP BY
    novel_id,
    grp;
...
CREATE TRIGGER trg_refresh_chapter_groups
    ...
    FOR EACH STATEMENT
    EXECUTE FUNCTION refresh_chapter_groups ();
