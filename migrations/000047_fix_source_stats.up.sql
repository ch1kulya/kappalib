...
UPDATE
    sources s
SET
    novels_count = COALESCE(stats.actual_novels_count, 0)
FROM
    actual_stats stats
WHERE
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        UPDATE
            sources s
        SET
            novels_count = COALESCE(d.actual_count, 0)
        FROM (
            SELECT
                c.source_id,
                COUNT(DISTINCT c.novel_id) AS actual_count
            FROM
                chapters c
            WHERE
                c.source_id IN (
...
CREATE TRIGGER trg_chapter_stats_ai
    AFTER INSERT ON chapters REFERENCING NEW TABLE AS new_table
    FOR EACH STATEMENT
    EXECUTE FUNCTION update_source_chapter_chapter_stats ();

CREATE TRIGGER trg_chapter_stats_ad
    AFTER DELETE ON chapters REFERENCING OLD TABLE AS old_table
    FOR EACH STATEMENT
    EXECUTE FUNCTION update_source_chapter_chapter_stats ();

CREATE TRIGGER trg_chapter_stats_au
    AFTER UPDATE ON chapters REFERENCING NEW TABLE AS new_table OLD TABLE AS old_table
    FOR EACH STATEMENT
    EXECUTE FUNCTION update_source_chapter_chapter_stats ();
