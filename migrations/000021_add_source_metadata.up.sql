ALTER TABLE sources ADD COLUMN novels_count INTEGER NOT NULL DEFAULT 0;
ALTER TABLE sources ADD COLUMN chapters_count INTEGER NOT NULL DEFAULT 0;
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        IF NEW.source_id IS NOT NULL THEN
            UPDATE sources SET chapters_count = chapters_count + 1 WHERE id = NEW.source_id;
    ...
    ELSIF TG_OP = 'DELETE' THEN
        IF OLD.source_id IS NOT NULL THEN
            UPDATE sources SET chapters_count = chapters_count - 1 WHERE id = OLD.source_id;

            IF NOT EXISTS (
                SELECT 1 FROM chapters
                WHERE novel_id = OLD.novel_id AND source_id = OLD.source_id
            ) THEN
                UPDATE sources SET novels_count = novels_count - 1 WHERE id = OLD.source_id;
            END IF;
        END IF;
        RETURN OLD;
...
UPDATE sources s SET
    chapters_count = COALESCE((SELECT COUNT(*) FROM chapters c WHERE c.source_id = s.id), 0),
    novels_count = COALESCE((SELECT COUNT(DISTINCT novel_id) FROM chapters c WHERE c.source_id = s.id), 0);
