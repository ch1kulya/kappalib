DROP TRIGGER IF EXISTS trg_source_stats_insert ON chapters;

DROP TRIGGER IF EXISTS trg_source_stats_delete ON chapters;

DROP TRIGGER IF EXISTS trg_source_stats_update ON chapters;

DROP FUNCTION IF EXISTS update_source_chapter_stats_insert ();

DROP FUNCTION IF EXISTS update_source_chapter_stats_delete ();

DROP FUNCTION IF EXISTS update_source_chapter_stats_update ();

DROP INDEX IF EXISTS idx_chapters_novel_id_source_id;
...
ELSIF TG_OP = 'DELETE' THEN
    IF OLD.source_id IS NOT NULL THEN
        ...
        UPDATE
            sources
        SET
            novels_count = GREATEST (0, novels_count - 1)
        WHERE
            id = OLD.source_id;
    END IF;
END IF;
...
CREATE OR REPLACE TRIGGER trg_update_source_chapter_stats
