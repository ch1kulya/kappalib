    ...
    IF TG_OP = 'INSERT' THEN
        WITH new_pairs AS (
            SELECT
                source_id,
                novel_id,
                COUNT(*) AS added_chapters,
                COALESCE(SUM(LENGTH(content)), 0) AS added_chars
            FROM
                new_rows
            WHERE
                source_id IS NOT NULL
...
source_deltas AS (
    SELECT
        source_id,
        SUM(added_chapters) AS chapters_delta,
        SUM(added_chars) AS characters_delta
    FROM
        new_pairs
    GROUP BY
        source_id
...
novels_delta AS (
    ...
    WHERE (
        ...
        FROM
            chapters c
        WHERE
            c.source_id = np.source_id
            AND c.novel_id = np.novel_id) - np.added_chapters = 0
    GROUP BY
        np.source_id)
UPDATE
    sources s
SET
    chapters_count = s.chapters_count + sd.chapters_delta,
...
WHERE
    ...
    UPDATE
        sources s
    SET
        novels_count = s.novels_count + nd.novels_delta
    FROM
        novels_delta nd
    WHERE
        s.id = nd.source_id;
...
ELSIF TG_OP = 'DELETE' THEN
    WITH old_pairs AS (
        SELECT
            source_id,
            novel_id,
            COUNT(*) AS rem_chapters,
            COALESCE(SUM(LENGTH(content)), 0) AS rem_chars
        FROM
            old_rows
        WHERE
            source_id IS NOT NULL
...
source_deltas AS (
    SELECT
        source_id,
        SUM(rem_chapters) AS chapters_delta,
        SUM(rem_chars) AS characters_delta
    FROM
        old_pairs
    GROUP BY
        source_id
...
novels_delta AS (
    ...
    WHERE (
        ...
        FROM
            chapters c
        WHERE
            c.source_id = op.source_id
            AND c.novel_id = op.novel_id) = 0
    GROUP BY
        op.source_id)
UPDATE
    sources s
SET
    chapters_count = GREATEST (0, s.chapters_count - sd.chapters_delta),
...
WHERE
    ...
    UPDATE
        sources s
    SET
        novels_count = GREATEST (0, s.novels_count - nd.novels_delta)
    FROM
        novels_delta nd
    WHERE
        s.id = nd.source_id;
...
UPDATE
    sources s
SET
    chapters_count = COALESCE(a.chapters, 0),
...
CREATE TRIGGER trg_update_source_chapter_stats_update
    AFTER UPDATE ON chapters REFERENCING NEW TABLE AS new_rows OLD TABLE AS old_rows
    FOR EACH STATEMENT
    EXECUTE FUNCTION update_source_chapter_stats ();
