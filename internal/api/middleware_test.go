...
func TestRateLimitMiddleware_ServiceTokenBypass(t *testing.T) {
	rl := NewRateLimiter()
	rl.apiToken = "test-secret-token"
	...
	for i := 0; i < defaultBurst+10; i++ {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		req.Header.Set("X-Real-IP", ip)
		req.Header.Set("X-Service-Token", "test-secret-token")
		rec := httptest.NewRecorder()
		handler.ServeHTTP(rec, req)

		if rec.Code != http.StatusOK {
			t.Fatalf("request %d: expected status 200 with service token, got %d", i+1, rec.Code)
		}
	}
}
